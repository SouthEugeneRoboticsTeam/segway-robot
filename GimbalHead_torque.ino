...
float MAX_SPEED = 9.0;  // max speed (in steps/sec) that the motors can run at (originally 1500)
...
float DRIVE_SPEED_SCALER = .75;  // what proportion of MAX_SPEED the robot's target driving speed can be-some extra speed must be kept in reserve to remain balanced
float TURN_SPEED_SCALER = .18;  // what proportion of MAX_SPEED can be given differently to each wheel in order to turn-controls maximum turn rate
...
uint8_t RightledR = A39; //GPIO12
uint8_t RightledG = A36; //GPIO14
uint8_t RightledB = A34; //GPIO27
uint8_t LeftledR = A35; //GPIO4
uint8_t LeftledG = A4; //GPIO0
uint8_t LeftledB = A2; //GPIO2
*/
int RED = 256;
int GREEN = 256;
int BLUE = 256;
...
*/
  ...
  motor1.initFOC(1.56, Direction::CCW);
  motor2.initFOC(3.71, Direction::CW);
...
//motorSpeed = 0;
 ...
 //   Serial.print(leftMotorWriteSpeed);
 //   Serial.print(" ");
 //   Serial.print(rightMotorWriteSpeed);
 //   Serial.println(" ");
//Serial.print(7);
/*
...
//Serial.print(9);
    ...
    oldTime = millis();
    //Serial.println(String(loopTime) + " " + String(leftMotorWriteSpeed));
    //Serial.print(" ");
    //Serial.println(leftMotorWriteSpeed);
...
//  turnSpeedVal = map(readByteFromBuffer(counter), 0, 200, -MAX_SPEED * TURN_SPEED_SCALER, MAX_SPEED * TURN_SPEED_SCALER);
  speedVal = ((float(readByteFromBuffer(counter)) - 200.0) * ((MAX_SPEED * DRIVE_SPEED_SCALER) - (-MAX_SPEED * DRIVE_SPEED_SCALER))) / (0.0 - 200.0) + (-MAX_SPEED * DRIVE_SPEED_SCALER);
  turnSpeedVal = ((float(readByteFromBuffer(counter)) - 0.0) * ((MAX_SPEED * TURN_SPEED_SCALER) - (-MAX_SPEED * TURN_SPEED_SCALER))) / (200.0 - 0.0) + (-MAX_SPEED * TURN_SPEED_SCALER);
